...
if page == "Company Workstation":
    ...
    with top1:
        tk = st.selectbox("Company lookup", sorted(UNIVERSE.keys()),
                          index=sorted(UNIVERSE.keys()).index("CCL"),
                          help="Universe is dropdown-restricted")
    master = build_master()
    row = master.loc[tk]
    with top2:
        sc = score_company(tk)
        st.write("")
        st.markdown(
            f"**Latest scores:**  Altman Z'' `{row['altman_z']:.2f}` {ZONE_ICON[row['altman_zone']]}  ·  "
            f"Piotroski `{int(row['piotroski_f']) if pd.notna(row['piotroski_f']) else 'n/a'}/9`  ·  "
            f"Beneish `{row['beneish_m']:.2f}`" if pd.notna(row['beneish_m']) else
            f"**Latest scores:**  Altman Z'' `{row['altman_z']:.2f}` {ZONE_ICON[row['altman_zone']]}")
    header_band(tk, row)

    module = st.radio("Analysis modules",
                      ["Overview", "Altman Z''", "Piotroski F", "Beneish M",
                       "Market Risk (Merton)", "ECL & Provisioning", "Outlook Report",
                       "SEC Filings"],
    ...
    if module == "Overview":
        ...
        with c1:
            ...
            snap = pd.DataFrame({
                ...
                "Value": [f"{row['altman_z']:.2f}" if pd.notna(row['altman_z']) else "n/a",
                          f"{int(row['piotroski_f'])}/9" if pd.notna(row['piotroski_f']) else "n/a",
                          f"{row['beneish_m']:.2f}" if pd.notna(row['beneish_m']) else "n/a",
                          f"{row['merton_dd']:.2f}" if pd.notna(row['merton_dd']) else "n/a",
                          f"{row['merton_PD']*100:.3f}%" if pd.notna(row['merton_PD']) else "n/a"],
                "Read": [row["altman_zone"],
                         "Strong" if pd.notna(row['piotroski_f']) and row['piotroski_f'] >= 7
                         else "Moderate" if pd.notna(row['piotroski_f']) and row['piotroski_f'] >= 4
                         else "Weak" if pd.notna(row['piotroski_f']) else "n/a",
                         "Flag" if pd.notna(row['beneish_m']) and row['beneish_m'] > -1.78
                         else "Clean" if pd.notna(row['beneish_m']) else "n/a",
                         "-", "-"]})
        ...
        with st.spinner("Building from SEC filings..."):
            fig = altman_chart(tk)
        st.pyplot(fig, use_container_width=True) if fig else st.info("Insufficient filing data.")
        ...
        st.pyplot(fig, use_container_width=True) if fig else \
        ...
        st.pyplot(fig, use_container_width=True) if fig else st.info("Quarterly series unavailable.")
        with st.spinner("Computing daily approximation..."):
            fig = merton_daily_chart(tk)
        st.pyplot(fig, use_container_width=True) if fig else st.info("Daily series unavailable.")
