...
st.sidebar.caption("Credit risk analysis on live SEC EDGAR + market data")
...
if page == "Company Deep Dive":
    ...
    with left:
        ...
        c5, c6 = st.columns(2)
        c5.metric("Rating PD (1yr)", f"{row['rating_PD']*100:.2f}%")
        c6.metric("Coverage", f"{row['provision']/row['EAD']*100:.2f}%")
    with right:
    ...
    with st.spinner("Building Piotroski deep dive..."):
        fig = piotroski_chart(tk)
    if fig:
        st.pyplot(fig, use_container_width=True)
    else:
        st.info("Piotroski time-series unavailable for this name.")
    with st.spinner("Computing quarterly Merton default probability..."):
        fig = merton_quarterly_chart(tk)
    if fig:
        st.pyplot(fig, use_container_width=True)
    else:
        st.info("Quarterly Merton series unavailable for this name.")
    ...
    st.header("Outlook Board")
    st.caption("Rule-based over/under-perform calls with a forensic veto — no "
               "high-conviction calls on financials the market distrusts. "
               "Directional view, not investment advice.")
    tk = st.selectbox("Company", sorted(UNIVERSE.keys()),
                      index=sorted(UNIVERSE.keys()).index("WDC") if "WDC" in UNIVERSE else 0)
    with st.spinner(f"Building outlook report for {tk}..."):
